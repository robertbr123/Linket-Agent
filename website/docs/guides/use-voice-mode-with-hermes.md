---
sidebar_position: 8
title: "Use Voice Mode with Linket"
description: "A practical guide to setting up and using Linket voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Linket
...
- you want Linket sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Linket.
...
## Step 1: make sure normal Linket works first
...
Linket supports both local and cloud speech stacks.
...
### If you use `linket setup`

If you choose NeuTTS in the setup wizard, Linket checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Linket transcribes and responds
...
- asking Linket to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Linket loop.

## Tuning CLI behavior

### Silence threshold

If Linket starts/stops too aggressively, tune:
...
Linket stays a normal chat bot, but can speak replies.
...
- you want Linket to function like a portable research or ops assistant
...
Linket joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Linket detects speech boundaries
- transcripts are posted in the associated text channel
- Linket responds in text and audio
...
1. get text Linket working
2. install `linket-agent[voice]`
