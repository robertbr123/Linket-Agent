...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Linket Agent — no API key required"
---

# MiniMax OAuth

Linket Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Linket automatically refreshes your session.
...
# → Linket opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.linket/auth.json` and are refreshed automatically before each session.
...
```bash
linket auth add minimax-oauth --region cn
```
...
```bash
linket auth add minimax-oauth --no-browser
```

Linket will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Linket implements a PKCE device-code flow against the MiniMax OAuth endpoints:

1. Linket generates a PKCE verifier / challenge pair and a random state value.
...
4. Linket polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.linket/auth.json` under the `minimax-oauth` key.
...
```bash
linket config set model MiniMax-M2.7
linket config set provider minimax-oauth
```

## Configuration Reference

After login, `~/.linket/config.yaml` will contain entries similar to:
...
```bash
linket --provider minimax-oauth    # canonical
linket --provider minimax-portal   # alias
linket --provider minimax-global   # alias
linket --provider minimax_oauth    # alias (underscore form)
```
...
```bash
HERMES_INFERENCE_PROVIDER=minimax-oauth linket
```
...
Linket refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Linket marks the session as requiring re-login.

**Fix:** run `linket auth add minimax-oauth` again to start a fresh login.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Linket raises a timeout error.

**Fix:** re-run `linket auth add minimax-oauth` (or `linket model`). The flow starts fresh.

### State mismatch (possible CSRF)

Linket detected that the `state` value returned by the authorization server does not match what it sent.
...
If `linket` cannot open a browser window, use `--no-browser`:

```bash
linket auth add minimax-oauth --no-browser
```

Linket prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `linket model` and select MiniMax (OAuth), or run `linket auth add minimax-oauth`.
...
```bash
linket auth remove minimax-oauth
```
...
- [linket doctor](../reference/cli-commands.md)
