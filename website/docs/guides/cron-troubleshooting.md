...
description: "Diagnose and fix common Linket cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`linket gateway` or `linket serve`). For one-off debugging, you can manually trigger a tick with `linket cron tick`.
...
```bash
date
linket cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.linket/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.linket/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.linket/.env` |
...
| `local` | Write access to `~/.linket/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `linket cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `linket skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `linket skills list`.
...
2. `~/.linket/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `linket cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Linket config directory). Verify:
```bash
ls ~/.linket/scripts/your-script.py   # Must exist
linket cron edit <job_id> --script ~/.linket/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `linket skills install <skill-name>`.
...
Jobs are stored in `~/.linket/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.linket/cron/jobs.json
chmod 600 ~/.linket/cron/jobs.json   # Your user should own it
```
...
```bash
linket cron list                    # Show all jobs, states, next_run times
linket cron run <job_id>            # Schedule for next tick (for testing)
linket cron edit <job_id>           # Fix configuration issues
linket logs                         # View recent Linket logs
linket skills list                  # Verify installed skills
```
...
1. Run the job with `linket cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.linket/logs/agent.log` for scheduler messages and `~/.linket/logs/errors.log` for warnings
3. Open an issue at [github.com/robertbr123/Linket-Agent](https://github.com/robertbr123/Linket-Agent) with:
