...
Copy-paste recipes for common automation patterns. Each template uses Linket's built-in [cron scheduler](/docs/user-guide/features/cron) for time-based triggers and [webhook platform](/docs/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`linket webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `linket webhook subscribe`) |
...
```bash
linket cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/linket-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/linket-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
linket webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
linket cron create "0 9 * * 1" \
  "Scan the NousResearch/linket-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/linket-agent --state merged --json number,title,files,mergedAt --limit 30
...
```bash
linket cron create "0 6 * * *" \
  "Run a dependency security audit on the linket-agent project.

1. cd ~/.linket/linket-agent && source .venv/bin/activate
...
```bash
linket webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
linket webhook subscribe alert-triage \
...
```python title="~/.linket/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Linket-Monitor/1.0"})
...
linket cron create "every 30m" \
  ...
  --script ~/.linket/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
linket cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Linket Agent development." \
...
```bash
linket webhook subscribe github-issues \
  --events "issues" \
...
```bash
linket webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
linket webhook subscribe stripe-payments \
...
```bash
linket cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the linket-agent codebase.
