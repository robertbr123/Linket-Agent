---
sidebar_position: 7
title: "Use SOUL.md with Linket"
description: "How to use SOUL.md to shape Linket Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Linket

`SOUL.md` is the **primary identity** for your Linket instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Linket to feel like the same assistant every time you talk to it — or if you want to replace the Linket persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Linket should be
- what Linket should avoid stylistically
- how Linket should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Linket is and how Linket speaks
...
Linket now uses only the global SOUL file for the current instance:

```text
~/.linket/SOUL.md
```

If you run Linket with a custom home directory, it becomes:
...
Linket automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Linket does not overwrite it
- if the file exists but is empty, Linket adds nothing from it to the prompt

## How Linket uses it

When Linket starts a session, it reads `SOUL.md` from `HERMES_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Linket feels.
...
Linket already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Linket should sound.

# Avoid
What Linket should not do.

# Defaults
How Linket should behave when ambiguity appears.
```
...
Then restart Linket or start a new session.
...
### I edited SOUL.md but Linket still sounds the same

Check:
- you edited `~/.linket/SOUL.md` or `$HERMES_HOME/SOUL.md`
...
### Linket is ignoring parts of my SOUL.md
