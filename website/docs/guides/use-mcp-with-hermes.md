---
sidebar_position: 6
title: "Use MCP with Linket"
description: "A practical guide to connecting MCP servers to Linket Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Linket

This guide shows how to actually use MCP with Linket Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Linket tool
- you want Linket to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Linket to internal APIs, databases, or company systems without modifying Linket core

Do not use MCP when:
- a built-in Linket tool already solves the job well
...
- Linket remains the agent
- MCP servers contribute tools
- Linket discovers those tools at startup or reload time
...
If you installed Linket with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Linket banner/status should show MCP integration when configured
- ask Linket what tools it has available
...
## WSL2: bridge Linket in WSL to Windows Chrome

This is the practical setup when:

- Linket runs inside WSL2
...
In this setup, Linket does **not** connect to Chrome directly. Instead:

- Linket runs in WSL
- Linket starts a local stdio MCP server
...
```text
Linket (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Linket stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Linket core browser transport
...
```bash
linket mcp add chrome-devtools-win --command cmd.exe --args /c "npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics"
```

After saving the server:

```bash
linket mcp test chrome-devtools-win
```

Then start a fresh Linket session or run:
...
Once loaded, Linket can use the MCP-prefixed browser tools directly. For example:
...
If Linket runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Linket from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Linket from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Linket:
...
2. Linket-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Linket will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Linket to reason over a bounded workspace.
...
Now Linket can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Linket core.
...
Because Linket now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
