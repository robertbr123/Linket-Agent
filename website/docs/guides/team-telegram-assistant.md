...
This tutorial walks you through setting up a Telegram bot powered by Linket Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Linket Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.linket/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Linket itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Linket Assistant`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Linket Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Linket Gateway...
...
linket gateway start
linket gateway stop
tail -f ~/.linket/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `linket gateway install` to update the plist.
:::
...
```bash
linket gateway stop && linket gateway start
```
...
linket pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.linket/.env`:
...
Control how much detail the bot shows when using tools. In `~/.linket/config.yaml`:
...
Customize how the bot communicates by editing `~/.linket/SOUL.md`:

For a full guide, see [Use SOUL.md with Linket](/docs/guides/use-soul-with-linket).
...
```bash
# From the CLI
linket cron list          # View all scheduled jobs
linket cron status        # Check if scheduler is running
...
| Cron job output | `~/.linket/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.linket/cron/jobs.json` |
| Pairing data | `~/.linket/pairing/` |
| Session history | `~/.linket/sessions/` |
