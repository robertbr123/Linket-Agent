...
sidebar_position: 7
title: "Docker"
description: "Running Linket Agent in Docker and using Docker as a terminal backend"
---

# Linket Agent — Docker

There are two distinct ways Docker intersects with Linket Agent:

1. **Running Linket IN Docker** — the agent itself runs inside a container (this page's primary focus)
2. **Docker as a terminal backend** — the agent runs on your host but executes every command inside a single, persistent Docker sandbox container that survives across tool calls, `/new`, and subagents for the life of the Linket process (see [Configuration → Docker Backend](./configuration.md#docker-backend))

This page covers option 1. The container stores all user data (config, API keys, sessions, skills, memories) in a single directory mounted from the host at `/opt/data`. The image itself is stateless and can be upgraded by pulling a new version without losing any configuration.

## Quick start

If this is your first time running Linket Agent, create a data directory on the host and start the container interactively to run the setup wizard:

```sh
mkdir -p ~/.linket
docker run -it --rm \
  -v ~/.linket:/opt/data \
  nousresearch/linket-agent setup
```

This drops you into the setup wizard, which will prompt you for your API keys and write them to `~/.linket/.env`. You only need to do this once. It is highly recommended to set up a chat system for the gateway to work with at this point.
...
docker run -d \
  ...
  -v ~/.linket:/opt/data \
  -p 8642:8642 \
  nousresearch/linket-agent gateway run
```
...
The entrypoint starts `linket dashboard` in the background (running as the non-root `linket` user) before `exec`-ing the main command. Dashboard output is prefixed with `[dashboard]` in `docker logs` so it's easy to separate from gateway logs.
...
The `/opt/data` volume is the single source of truth for all Linket state. It maps to your host's `~/.linket/` directory and contains:

| Path | Contents |
|------|----------|
| `.env` | API keys and secrets |
| `config.yaml` | All Linket configuration |
...
:::warning
Never run two Linket **gateway** containers against the same data directory simultaneously — session files and memory stores are not designed for concurrent write access.
:::

## Multi-profile support

Linket supports [multiple profiles](../reference/profile-commands.md) — separate `~/.linket/` directories that let you run independent agents (different SOUL, skills, memory, sessions, credentials) from a single installation. **When running under Docker, using Linket' built-in multi-profile feature is not recommended.**
...
docker run -d \
  --name linket-work \
  --restart unless-stopped \
  -v ~/.linket-work:/opt/data \
  -p 8642:8642 \
  nousresearch/linket-agent gateway run

# Personal profile
docker run -d \
  --name linket-personal \
  --restart unless-stopped \
  -v ~/.linket-personal:/opt/data \
  -p 8643:8642 \
  nousresearch/linket-agent gateway run
```
...
- **Independent lifecycle** — upgrade, restart, pause, or roll back each agent separately (`docker restart linket-work` leaves `linket-personal` untouched).
...
services:
  linket-work:
    image: nousresearch/linket-agent:latest
    container_name: linket-work
    restart: unless-stopped
    command: gateway run
    ports:
      - "8642:8642"
    volumes:
      - ~/.linket-work:/opt/data

  linket-personal:
    image: nousresearch/linket-agent:latest
    container_name: linket-personal
    restart: unless-stopped
    command: gateway run
    ports:
      - "8643:8642"
    volumes:
      - ~/.linket-personal:/opt/data
```
...
The Linket container needs moderate resources. Recommended minimums:
...
- Python 3 with all Linket dependencies (`uv pip install -e ".[all]"`)
...
- Optionally launches `linket dashboard` as a background side-process when `HERMES_DASHBOARD=1` (see [Running the dashboard](#running-the-dashboard))
- Then runs `linket` with whatever arguments you pass
...
```sh
docker pull nousresearch/linket-agent:latest
docker rm -f linket
docker run -d \
  --name linket \
  --restart unless-stopped \
...
When using Docker as the execution environment (not the methods above, but when the agent runs commands inside a Docker sandbox — see [Configuration → Docker Backend](./configuration.md#docker-backend)), Linket reuses a single long-lived container for all tool calls and automatically bind-mounts the skills directory (`~/.linket/skills/`) and any credential files declared by skills into that container as read-only volumes. Skill scripts, templates, and references are available inside the sandbox without manual configuration, and because the container persists for the life of the Linket process, any dependencies you install or files you write stay around for the next tool call.

The same syncing happens for SSH and Modal backends — skills and credential files are uploaded via rsync or the Modal mount API before each command.

## Connecting to local inference servers (vLLM, Ollama, etc.)

When running Linket in Docker and your inference server (vLLM, Ollama, text-generation-inference, etc.) is also running on the host or in another container, networking requires extra attention.
...
Then in your `~/.linket/config.yaml`, use the **container name** as the hostname:
...
:::tip Key points
- Use the **container name** (`vllm`) as the hostname — not `localhost` or `127.0.0.1`, which refer to the Linket container itself.
...
From inside the Linket container, confirm the inference server is reachable:

```sh
docker exec linket curl -s http://vllm:8000/v1/models
```

You should see a JSON response listing your served model. If this fails, check:

1. Both containers are on the same Docker network (`docker network inspect linket-net`)
...
Check logs: `docker logs linket`. Common causes:
- Missing or invalid `.env` file — run interactively first to complete setup
- Port conflicts if running with exposed ports

### "Permission denied" errors

The container's entrypoint drops privileges to the non-root `linket` user (UID 10000) via `gosu`. If your host `~/.linket/` is owned by a different UID, set `HERMES_UID`/`HERMES_GID` to match your host user, or ensure the data directory is writable:
...
```sh
docker restart linket
```

### Checking container health

```sh
docker logs --tail 50 linket          # Recent logs
docker run -it --rm nousresearch/linket-agent:latest version     # Verify version
docker stats linket                    # Resource usage
```
