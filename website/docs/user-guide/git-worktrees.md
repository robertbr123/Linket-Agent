...
title: "Git Worktrees"
description: "Run multiple Linket agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Linket Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Linket so each session has a clean, isolated working directory.

## Why Use Worktrees with Linket?

Linket treats the **current working directory** as the project root:

- CLI: the directory where you run `linket` or `linket chat`
...
git worktree add ../repo-feature feature/linket-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/linket-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Linket there:

```bash
cd ../repo-feature

# Start Linket in the worktree
linket
```
...
git worktree add ../repo-experiment-a feature/linket-a
git worktree add ../repo-experiment-b feature/linket-b
```
...
- Works on its own branch (`feature/linket-a` vs `feature/linket-b`).
...
- Linket checkpoint data under `~/.linket/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Linket experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/linket-checkpoints-docs`, `feature/linket-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Linket from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `linket -w` (Automatic Worktree Mode)

Linket has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `linket/linket-<hash>`).
...
```bash
linket -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `linket -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Linket session its own clean checkout.
