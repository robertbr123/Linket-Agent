...
Use the Teams meeting pipeline when you want Linket to ingest Microsoft Graph meeting events, fetch transcripts first, fall back to recordings plus STT when needed, and deliver a structured summary to downstream sinks.
...
Add Graph app-only credentials to `~/.linket/.env`:
...
Use this when you want Linket to post the summary through Microsoft Graph into a Teams chat or channel.
...
Start Linket normally after updating config:

```bash
linket gateway run
```

Or, if you run Linket in Docker, start the gateway the same way you already do for your deployment.
...
Microsoft Graph caps webhook subscriptions at 72 hours and will not auto-renew them. You MUST schedule `linket teams-pipeline maintain-subscriptions` before going live, or notifications will silently stop three days after any manual subscription creation. See [Automating subscription renewal](/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production) in the operator runbook — three options (Linket cron, systemd timer, plain crontab).
...
| Jobs do not appear in `linket teams-pipeline list` | Confirm `msgraph_webhook` is enabled and that subscriptions point at the right notification URL |
