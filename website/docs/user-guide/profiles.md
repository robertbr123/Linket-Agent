...
Run multiple independent Linket agents on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.

## What are profiles?

A profile is a separate Linket home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing up Linket state.
...
```bash
linket profile create coder       # creates profile + "coder" command alias
coder setup                       # configure API keys and model
coder chat                        # start chatting
```

That's it. `coder` is now its own Linket profile with its own config, memory, and state.
...
Copies your current profile's `config.yaml`, `.env`, and `SOUL.md` into the new profile. Same API keys and model, but fresh sessions and memory. Edit `~/.linket/profiles/work/.env` for different API keys, or `~/.linket/profiles/work/SOUL.md` for a different personality.
...
```bash
linket profile create work --clone --clone-from coder
```
...
The alias works with every linket subcommand — it's just `linket -p <name>` under the hood.
...
```bash
linket -p coder chat
linket --profile=coder doctor
linket chat -p coder -q "hello"    # works in any position
```

### Sticky default (`linket profile use`)

```bash
linket profile use coder
linket chat                   # now targets coder
linket tools                  # configures coder's tools
linket profile use default    # switch back
```

Sets a default so plain `linket` commands target that profile. Like `kubectl config use-context`.
...
- **`linket profile`**: Shows current profile name, path, model, gateway status

## Profiles vs workspaces vs sandboxing

Profiles are often confused with workspaces or sandboxes, but they are different things:

- A **profile** gives Linket its own state directory: `config.yaml`, `.env`, `SOUL.md`, sessions, memory, logs, cron jobs, and gateway state.
...
Using `cwd: "."` on the local backend means "the directory Linket was launched from", not "the profile directory".
...
```bash
# Edit coder's tokens
nano ~/.linket/profiles/coder/.env

# Edit assistant's tokens
nano ~/.linket/profiles/assistant/.env
```
...
```bash
coder config set model.default anthropic/claude-sonnet-4
echo "You are a focused coding assistant." > ~/.linket/profiles/coder/SOUL.md
```
...
`linket update` pulls code once (shared) and syncs new bundled skills to **all** profiles automatically:
...
```bash
linket profile list           # show all profiles with status
linket profile show coder     # detailed info for one profile
linket profile rename coder dev-bot   # rename (updates alias + service)
linket profile export coder   # export to coder.tar.gz
linket profile import coder.tar.gz   # import from archive
```

## Deleting a profile

```bash
linket profile delete coder
```

This stops the gateway, removes the systemd/launchd service, removes the command alias, and deletes all profile data. You'll be asked to type the profile name to confirm.

Use `--yes` to skip confirmation: `linket profile delete coder --yes`

:::note
You cannot delete the default profile (`~/.linket`). To remove everything, use `linket uninstall`.
:::
...
```bash
# Bash
eval "$(linket completion bash)"

# Zsh
eval "$(linket completion zsh)"
```
...
The default profile is simply `~/.linket` itself. No migration needed — existing installs work identically.
