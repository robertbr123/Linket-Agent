...
| | |
|---|---|
| Source | Optional — install with `linket skills install official/productivity/here-now` |
...
bash "$PUBLISH" {file-or-dir} --client linket
```
...
```bash
PUBLISH="${HERMES_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client linket
```
...
| `--client {name}`      | Agent name for attribution (e.g. `linket`)    |
