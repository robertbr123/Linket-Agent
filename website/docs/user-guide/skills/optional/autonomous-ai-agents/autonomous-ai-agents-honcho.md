...
description: "Configure and use Honcho memory with Linket -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Linket -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `linket skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Linket

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Linket profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/linket#running-honcho-locally-with-linket

### Verify

```bash
linket honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Linket profile's AI peer
...
Honcho models conversations as interactions between **peers**. Linket creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Linket instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Linket run |
| `global` | Single session across all directories |

Manual override: `linket honcho map my-project-name`
...
Each Linket profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block linket.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `linket.coder` host block in `honcho.json`
...
```bash
linket honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Linket when Honcho memory is active.
...
Run `linket honcho setup`. Ensure `memory.provider: honcho` is in `~/.linket/config.yaml`.

### Memory not persisting across sessions
Check `linket honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `linket profile create <name> --clone`. For existing profiles: `linket honcho sync`.
...
| Command | Description |
|---------|-------------|
| `linket honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `linket honcho status` | Show resolved config, connection test, peer info for active profile |
| `linket honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `linket honcho disable` | Disable Honcho for the active profile |
| `linket honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `linket honcho peers` | Show peer identities across all profiles |
| `linket honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `linket honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `linket honcho sessions` | List known directory-to-session-name mappings |
| `linket honcho map <name>` | Map current working directory to a Honcho session name |
| `linket honcho identity` | Seed AI peer identity or show both peer representations |
| `linket honcho sync` | Create host blocks for all Linket profiles that don't have one yet |
| `linket honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Linket + Honcho |
| `linket memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `linket memory status` | Show active memory provider and config |
| `linket memory off` | Disable external memory provider |
