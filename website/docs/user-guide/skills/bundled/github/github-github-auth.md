...
- Give it a name like "linket-agent"
- Select scopes:
...
- Give it a title like "linket-agent-&lt;machine-name>"
...
elif [ -f ~/.linket/.env ] && grep -q "^GITHUB_TOKEN=" ~/.linket/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.linket/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
