...
description: "Customize Linket Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Linket Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.
...
If you want to change who Linket is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Linket now seeds a default `SOUL.md` automatically in:

```text
~/.linket/SOUL.md
```

More precisely, it uses the current instance's `HERMES_HOME`, so if you run Linket with a custom home directory, it will use:
...
- Linket creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Linket loads `SOUL.md` only from `HERMES_HOME`
- Linket does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Linket falls back to a built-in default identity
...
If Linket loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HERMES_HOME`, the personality belongs to the Linket instance itself.

That also makes it easier to teach users:
- "Edit `~/.linket/SOUL.md` to change Linket' default personality."
...
- how Linket should handle uncertainty, disagreement, or ambiguity
...
## What Linket injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Linket falls back to a built-in default identity ("You are Linket Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Linket ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Linket, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Linket its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.linket/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.linket/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-linket)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Linket speaks
- `display.skin` and `/skin` affect how Linket looks in the terminal
