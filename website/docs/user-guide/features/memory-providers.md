...
Linket Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
linket memory setup      # interactive picker + configuration
linket memory status     # check what's active
linket memory off        # disable external provider
```

You can also select the active memory provider via `linket plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.linket/config.yaml`:
...
When a memory provider is active, Linket automatically:
...
**Setup Wizard:**
```bash
linket honcho setup        # (legacy command) 
# or
linket memory setup        # select "honcho"
```

**Config:** `$HERMES_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$HERMES_HOME/honcho.json` > `~/.linket/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/linket-ai/linket-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/linket).
...
:::tip Migrating from `linket honcho`
If you previously used `linket honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Linket profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Linket profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Linket profile. Host key `linket` → default; `linket.<profile>` for others. |
...
```bash
linket profile create coder --clone
```

`--clone` creates a `linket.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Linket profile, creates host blocks for any profile without one, inherits settings from the default `linket` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
See the [config reference](https://github.com/linket-ai/linket-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/linket).
...
linket memory setup    # select "openviking"
# Or manually:
linket config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.linket/.env
```
...
**Setup:**
```bash
linket memory setup    # select "mem0"
# Or manually:
linket config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.linket/.env
```
...
| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `linket-user` | User identifier |
| `agent_id` | `linket` | Agent identifier |
...
**Setup:**
```bash
linket memory setup    # select "hindsight"
# Or manually:
linket config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.linket/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p linket ui start`
...
| `bank_id` | `linket` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Linket Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/robertbr123/Linket-Agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
linket memory setup    # select "holographic"
# Or manually:
linket config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.linket-memory-store`
...
**Setup:**
```bash
linket memory setup    # select "retaindb"
# Or manually:
linket config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.linket/.env
```
...
# Then configure Linket
linket memory setup    # select "byterover"
# Or manually:
linket config set memory.provider byterover
```
...
**Setup:**
```bash
linket memory setup    # select "supermemory"
# Or manually:
linket config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.linket/.env
```
...
| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `linket` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `linket-{identity}` → `linket-coder`) to isolate memories per Linket profile
