...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Linket Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Linket Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Linket](/docs/guides/use-voice-mode-with-linket).

## Prerequisites

Before using voice features, make sure you have:

1. **Linket Agent installed** — `pip install linket-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `linket model` or set your preferred provider credentials in `~/.linket/.env`
3. **A working base setup** — run `linket` to verify the agent responds to text before enabling voice

:::tip
The `~/.linket/` directory and default `config.yaml` are created automatically the first time you run `linket`. You only need to create `~/.linket/.env` manually for API keys.
:::
...
pip install "linket-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "linket-agent[all]"
```
...
Voice mode is available in both the **classic CLI** (`linket chat`) and the **TUI** (`linket --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.linket/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
linket                # Start the interactive CLI
```
...
1. Start the CLI with `linket` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.linket/config.yaml` (default: `ctrl+b`).
:::
...
```bash
linket gateway        # Start the gateway (connects to configured platforms)
linket gateway setup  # Interactive setup wizard for first-time configuration
```
...
:::tip
To disable the mention requirement in server channels, add to `~/.linket/.env`:
```bash
...
```bash
linket gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.linket/.env`
...
- Review gateway logs: `tail -f ~/.linket/logs/gateway.log`
