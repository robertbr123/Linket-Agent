...
description: "Plugins shipped with Linket Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Linket ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.linket/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/docs/user-guide/features/plugins) page for the general plugin system, and [Build a Linket Plugin](/docs/guides/build-a-linket-plugin) to write your own.
...
2. **User** — `~/.linket/plugins/<name>/`
3. **Project** — `./.linket/plugins/<name>/` (requires `HERMES_ENABLE_PROJECT_PLUGINS=1`)
...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `linket memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `linket plugins list` and the interactive `linket plugins` UI), but none load until you explicitly enable them:

```bash
linket plugins enable disk-cleanup
```

Or via `~/.linket/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Linket. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
linket plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `linket plugins enable <name>`.
...
| `linket-achievements` | dashboard tab | Steam-style collectible badges generated from your real Linket session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `linket memory` and `linket plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `HERMES_HOME` or `/tmp/linket-*`, track it silently as `test` / `temp` / `cron-output`. |
...
**Safety** — cleanup only ever touches paths under `HERMES_HOME` or `/tmp/linket-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `linket plugins enable disk-cleanup` (or check the box in `linket plugins`).

**Disabling again:** `linket plugins disable disk-cleanup`.

### observability/langfuse

Traces Linket turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Linket' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `linket logs`.
...
```bash
linket tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Linket and the next turn ships a trace.

**Setup (manual):**

```bash
pip install langfuse
linket plugins enable observability/langfuse
```

Then put the credentials in `~/.linket/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Linket turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
Session grouping keys off the Linket session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `linket chat` session lives under one Langfuse session.

**Verify:**

```bash
linket plugins list                 # observability/langfuse should show "enabled"
linket chat -q "hello"              # check the Langfuse UI for a "Linket turn" trace
```
...
Linket-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Linket-prefixed wins when both are set.
...
**Disabling:** `linket plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.linket/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
linket plugins enable google_meet
...
**Disabling:** `linket plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.linket/cache/google_meet/` until you remove them.

### linket-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Linket session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Linket feature changes.

**How it works:**

- Scans your entire `~/.linket/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$HERMES_HOME/plugins/linket-achievements/state.json`
...
| Secret | Hidden until Linket detects the first related signal in your history |

**API** — routes mount under `/api/plugins/linket-achievements/`:
...
**State files** — live under `$HERMES_HOME/plugins/linket-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Linket updates. |
...
**Enabling:** Nothing to enable — `linket-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `linket dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/linket-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.linket/plugins/linket-achievements/` that ships no dashboard. The plugin's state files under `$HERMES_HOME/plugins/linket-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Linket plugin — see [Build a Linket Plugin](/docs/guides/build-a-linket-plugin). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.linket/plugins/<name>/`
- Manifest source is reported as `bundled` in `linket plugins list`
