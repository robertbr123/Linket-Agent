...
description: "Install and deploy Linket Agent with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---
...
That's it. `nixos-rebuild switch` creates the `linket` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
:::info
When `container.enable = true` and `addToSystemPackages = true`, **every** `linket` command on the host automatically routes into the managed container. This means your interactive CLI session runs inside the same environment as the gateway service — with access to all container-installed packages and tools.
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$HERMES_HOME/.env` at activation time (`nixos-rebuild switch`). Linket reads this file on every startup, so changes take effect with a `systemctl restart linket-agent` — no container recreation needed.
...
Set `auth = "oauth"` for servers using OAuth 2.1. Linket implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.
...
**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u linket` (native):
...
| Blocked command | Why |
|---|---|
| `linket setup` | Config is declarative — edit `settings` in your Nix config |
...
2. **`.managed` marker file** in `HERMES_HOME` — set by the activation script, visible to interactive shells (e.g., `docker exec -it linket-agent linket config set ...` is also blocked)
...
Host                                    Container
────                                    ─────────
/nix/store/...-linket-agent-0.1.0  ──►  /nix/store/... (ro)
~/.linket -> /var/lib/linket/.linket       (symlink bridge, per hostUsers)
/var/lib/linket/                    ──►  /data/          (rw)
...
The NixOS module supports declarative plugin installation — no imperative `linket plugins install` needed.
...
| `/home/linket` | `${stateDir}/home` | `rw` | Persistent agent home — `pip install --user`, tool caches |
...
| `unable to find user linket` | Container still starting (entrypoint hasn't created user yet) | Wait a few seconds and retry — the CLI retries automatically |
