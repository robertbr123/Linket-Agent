...
description: "How Linket stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
Jobs are stored in `~/.linket/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `linket cron` commands are run or during active CLI sessions.
...
```python
# ~/.linket/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.linket/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `linket cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `linket cron` CLI provides direct job management:

```bash
linket cron list                    # Show all jobs
linket cron create                  # Interactive job creation (alias: add)
linket cron edit <job_id>           # Edit job configuration
linket cron pause <job_id>          # Pause a running job
linket cron resume <job_id>         # Resume a paused job
linket cron run <job_id>            # Trigger immediate execution
linket cron remove <job_id>         # Delete a job
```
