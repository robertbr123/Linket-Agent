...
description: "How to build a memory provider plugin for Linket Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Linket Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/docs/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `linket plugins`.
:::
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `linket memory setup` | **Yes** |
...
`get_config_schema()` returns a list of field descriptors used by `linket memory setup`:
...
data_dir = Path("~/.linket/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `linket my-provider status`, `linket my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `linket <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `linket --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: linket my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the linket my-provider argparse tree.
