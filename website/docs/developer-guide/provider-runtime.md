...
description: "How Linket resolves providers, credentials, API modes, and auxiliary models at runtime"
---

# Provider Runtime Resolution

Linket has a shared provider runtime resolver used across:
...
That ordering matters because Linket treats the saved model/provider choice as the source of truth for normal runs. This prevents a stale shell export from silently overriding the endpoint a user last selected in `linket model`.
...
This resolver is the main reason Linket can share auth/runtime logic between:
...
Set `AI_GATEWAY_API_KEY` in `~/.linket/.env` and run with `--provider ai-gateway`. Linket fetches available models from the gateway's `/models` endpoint, filtering to language models with tool-use support.

## OpenRouter, AI Gateway, and custom OpenAI-compatible base URLs

Linket contains logic to avoid leaking the wrong API key to a custom endpoint when multiple provider keys exist (e.g. `OPENROUTER_API_KEY`, `AI_GATEWAY_API_KEY`, and `OPENAI_API_KEY`).
...
Linket also distinguishes between:
...
When provider resolution selects `anthropic`, Linket uses:
...
- Linket preflights Anthropic credential refresh before native Messages API calls
- Linket still retries once on a 401 after rebuilding the Anthropic client, as a fallback path
...
When an auxiliary task is configured with provider `main`, Linket resolves that through the same shared runtime path as normal chat. In practice that means:

- env-driven custom endpoints still work
- custom endpoints saved via `linket model` / `config.yaml` also work
- auxiliary routing can tell the difference between a real saved custom endpoint and the OpenRouter fallback

## Fallback models

Linket supports a configured fallback model/provider pair, allowing runtime failover when the primary model encounters errors.
