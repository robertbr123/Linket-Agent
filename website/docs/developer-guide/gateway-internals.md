...
The messaging gateway is the long-running process that connects Linket to 14+ external messaging platforms through a unified architecture.
...
|--------|-----------------|
| `~/.linket/.env` | API keys, bot tokens, platform credentials |
| `~/.linket/config.yaml` | Model settings, tool configuration, display options |
...
Hooks are discovered from `gateway/builtin_hooks/` (always active) and `~/.linket/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `linket gateway start` / `linket gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.linket/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `linket gateway stop` stops only the current profile's gateway. `linket gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
