...
description: "How to build an image-generation backend plugin for Linket Agent"
---
...
:::tip
Image-gen is one of several **backend plugins** Linket supports. The others (with more specialized ABCs) are [Memory Provider Plugins](/docs/developer-guide/memory-provider-plugin), [Context Engine Plugins](/docs/developer-guide/context-engine-plugin), and [Model Provider Plugins](/docs/developer-guide/model-provider-plugin). General tool/hook/CLI plugins live in [Build a Linket Plugin](/docs/guides/build-a-linket-plugin).
:::

## How discovery works

Linket scans for image-gen backends in three places:

1. **Bundled** — `<repo>/plugins/image_gen/<name>/` (auto-loaded with `kind: backend`, always available)
2. **User** — `~/.linket/plugins/image_gen/<name>/` (opt-in via `plugins.enabled`)
3. **Pip** — packages declaring a `hermes_agent.plugins` entry point

Each plugin's `register(ctx)` function calls `ctx.register_image_gen_provider(...)` — that puts it into the registry in `agent/image_gen_registry.py`. The active provider is picked by `image_gen.provider` in `config.yaml`; `linket tools` walks users through selection.

The `image_generate` tool wrapper asks the registry for the active provider and dispatches there. If no provider is registered, the tool surfaces a helpful error pointing at `linket tools`.
...
A bundled plugin is complete at this point. User plugins at `~/.linket/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `linket plugins enable <name>`).
...
class MyBackendImageGenProvider(ImageGenProvider):
    ...
    def display_name(self) -> str:
        # Human label shown in `linket tools`. Defaults to name.title() if omitted.
        return "My Backend"
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        # Catalog shown in `linket tools` model picker.
        return [
            {
                "id": "my-model-fast",
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        # Metadata for the `linket tools` picker — keys to prompt for at setup.
        return {
            "name": "My Backend",
...
`kind: backend` is what routes the plugin to the image-gen registration path. `requires_env` is prompted during `linket plugins install`.
...
| `display_name` | — | `name.title()` | Label shown in `linket tools` |
| `is_available()` | — | `True` | Gate for missing creds/deps |
| `list_models()` | — | `[]` | Catalog for `linket tools` model picker |
...
Drop a user plugin at `~/.linket/plugins/image_gen/<name>/` with the same `name` property as a bundled one and enable it via `linket plugins enable <name>` — the registry is last-writer-wins, so your version replaces the built-in. Useful for pointing an `openai` plugin at a private proxy, or swapping in a custom model catalog.

## Testing

```bash
export HERMES_HOME=/tmp/linket-imggen-test
...
linket plugins enable my-backend
...
# Exercise it
linket -z "Generate an image of a corgi in a spacesuit"
```

Or interactively: `linket tools` → "Image Generation" → select `my-backend` → enter API key if prompted.
...
`my_backend_imggen_package` must expose a top-level `register` function. See [Distribute via pip](/docs/guides/build-a-linket-plugin#distribute-via-pip) in the general plugin guide for the full setup.
...
- [Build a Linket Plugin](/docs/guides/build-a-linket-plugin) — general tools/hooks/slash commands guide
