...
The ACP adapter wraps Linket' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
linket acp / linket-acp / python -m acp_adapter
...
- `allow_once` -> Linket `once`
- `allow_always` -> Linket `always`
- reject options -> Linket `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Linket tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["linket-acp"])
...
Instead it reuses Linket' runtime resolver:

- `acp_adapter/auth.py`
- `hermes_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Linket provider/credentials.
...
- ACP sessions are persisted to the shared `~/.linket/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `linket-acp` toolset definition
- `hermes_cli/main.py` — `hermes acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `linket-acp` script
