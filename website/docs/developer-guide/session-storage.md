...
Linket Agent uses a SQLite database (`~/.linket/state.db`) to persist session
...
```
~/.linket/state.db (SQLite, WAL mode)
...
Multiple linket processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.linket/state.db
...
Default path: `~/.linket/state.db`

This is derived from `hermes_constants.get_hermes_home()` which resolves to
`~/.linket/` by default, or the value of `HERMES_HOME` environment variable.
