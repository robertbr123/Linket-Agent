...
description: "How Linket builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Linket deliberately separates:
...
```
# Layer 1: Agent Identity (from ~/.linket/SOUL.md)
You are Linket, an AI assistant created by Nous Research.
...
`SOUL.md` lives at `~/.linket/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
```
You are Linket Agent, an intelligent AI assistant created by Nous Research.
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_hermes_md(cwd_path)       # 1. .linket.md / HERMES.md (walks to git root)
...
|----------|-------|-------------|-------|
| 1 | `.linket.md`, `HERMES.md` | CWD up to git root | Linket-native project config |
...
- **YAML frontmatter stripped** — `.linket.md` frontmatter is removed (reserved for future config overrides)
...
1. `.linket.md` / `HERMES.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Linket already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.linket/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.linket/MEMORY.md` and `~/.linket/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.linket.md`, `HERMES.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking Linket.
...
- if you want to change how Linket assembles prompts for everyone, change Python and treat it as a code contribution
