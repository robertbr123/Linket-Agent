...
description: "Reference for Linket core, composite, platform, and dynamic toolsets"
---
...
- **Platform** — A complete tool configuration for a specific deployment context (e.g., `linket-cli` is the default for interactive CLI sessions)
...
```bash
linket chat --toolsets web,file,terminal
linket chat --toolsets debugging        # composite — expands to file + terminal + web
linket chat --toolsets all              # everything
```
...
```yaml
toolsets:
  - linket-cli          # default for CLI
  # - linket-telegram   # override for Telegram gateway
```

### Interactive management

```bash
linket tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call Linket tools programmatically. |
| `cronjob` | `cronjob` | Schedule and manage recurring tasks. |
| `debugging` | composite (`file` + `terminal` + `web`) | Debug bundle — file, process/terminal, web extract/search. |
| `delegation` | `delegate_task` | Spawn isolated subagent instances for parallel work. |
| `discord` | `discord` | Core Discord text/embed/DM actions (gateway-only). Active on the `linket-discord` toolset. |
| `discord_admin` | `discord_admin` | Discord moderation (bans, role changes, channel management). Active on the `linket-discord` toolset; requires the bot to hold the relevant Discord permissions. |
...
| `feishu_drive` | `feishu_drive_add_comment`, `feishu_drive_list_comments`, `feishu_drive_list_comment_replies`, `feishu_drive_reply_comment` | Feishu/Lark drive comment operations. Scoped to the comment agent; not exposed on `linket-cli` or other messaging toolsets. |
...
| `yuanbao` | `yb_query_group_info`, `yb_query_group_members`, `yb_search_sticker`, `yb_send_dm`, `yb_send_sticker` | Yuanbao DM/group actions and sticker search. Registered only on `linket-yuanbao`. |

## Platform Toolsets

Platform toolsets define the complete tool configuration for a deployment target. Most messaging platforms use the same set as `linket-cli`:

| Toolset | Differences from `linket-cli` |
|---------|-------------------------------|
| `linket-cli` | Full toolset — 38 tools. The default for interactive CLI sessions. |
| `linket-acp` | Drops `clarify`, `cronjob`, `image_generate`, `send_message`, `text_to_speech`, and all four Home Assistant tools. Focused on coding tasks in IDE context. |
| `linket-api-server` | Drops `clarify`, `send_message`, and `text_to_speech`. Keeps everything else — suitable for programmatic access where user interaction isn't possible. |
| `linket-cron` | Same as `linket-cli`. |
| `linket-telegram` | Same as `linket-cli`. |
| `linket-discord` | Adds `discord` and `discord_admin` on top of `linket-cli`. |
| `linket-slack` | Same as `linket-cli`. |
| `linket-whatsapp` | Same as `linket-cli`. |
| `linket-signal` | Same as `linket-cli`. |
| `linket-matrix` | Same as `linket-cli`. |
| `linket-mattermost` | Same as `linket-cli`. |
| `linket-email` | Same as `linket-cli`. |
| `linket-sms` | Same as `linket-cli`. |
| `linket-bluebubbles` | Same as `linket-cli`. |
| `linket-dingtalk` | Same as `linket-cli`. |
| `linket-feishu` | Adds the five `feishu_doc_*` / `feishu_drive_*` tools (only used by the document-comment handler, not the regular chat adapter). |
| `linket-qqbot` | Same as `linket-cli`. |
| `linket-wecom` | Same as `linket-cli`. |
| `linket-wecom-callback` | Same as `linket-cli`. |
| `linket-weixin` | Same as `linket-cli`. |
| `linket-yuanbao` | Adds the five `yb_*` tools (DM/group/sticker) on top of `linket-cli`. |
| `linket-homeassistant` | Same as `linket-cli` (the Home Assistant tools are already present by default and activate when `HASS_TOKEN` is set). |
| `linket-webhook` | Same as `linket-cli`. |
...
## Relationship to `linket tools`

The `linket tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
