...
description: "Catalog of bundled skills that ship with Linket Agent"
---

# Bundled Skills Catalog

Linket ships with a large built-in skill library copied into `~/.linket/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.

Linket also syncs bundled skills on `linket update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.linket/skills/` tree, it is still shipped with Linket; restore it with `linket skills reset <name> --restore`.
...
| [`linket-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-linket-agent) | Configure, extend, or contribute to Linket Agent. | `autonomous-ai-agents/linket-agent` |
...
| [`kanban-worker`](/docs/user-guide/skills/bundled/devops/devops-kanban-worker) | Pitfalls, examples, and edge cases for Linket Kanban workers. The lifecycle itself is auto-injected into every worker's system prompt as KANBAN_GUIDANCE (from agent/prompt_builder.py); this skill is what you load when you want deeper det... | `devops/kanban-worker` |
...
| Skill | Description | Path |
|-------|-------------|------|
| [`debugging-linket-tui-commands`](/docs/user-guide/skills/bundled/software-development/software-development-debugging-linket-tui-commands) | Debug Linket TUI slash commands: Python, gateway, Ink UI. | `software-development/debugging-linket-tui-commands` |
| [`linket-agent-skill-authoring`](/docs/user-guide/skills/bundled/software-development/software-development-linket-agent-skill-authoring) | Author in-repo SKILL.md: frontmatter, validator, structure. | `software-development/linket-agent-skill-authoring` |
...
| [`plan`](/docs/user-guide/skills/bundled/software-development/software-development-plan) | Plan mode: write markdown plan to .linket/plans/, no exec. | `software-development/plan` |
