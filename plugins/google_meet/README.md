...
Let the linket agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where linket runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `linket meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `linket meet` CLI |
...
| `cli.py` | `linket meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/cli.py` | v3: `linket meet node {run,list,approve,remove,status,ping}` |
...
linket meet install                                      # pip + Chromium
linket meet setup                                        # preflight
linket meet auth                                         # optional
linket meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
linket meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.linket/.env
linket meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
linket meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
linket meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.linket/.env
linket meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, linket will **not** switch your system audio input automatically — the
...
linket plugins enable google_meet
linket meet node run --display-name my-mac --host 0.0.0.0 --port 18789
# prints the bearer token on first run; copy it
```

On the gateway:
```bash
linket meet node approve my-mac ws://<mac-ip>:18789 <token>
linket meet node ping my-mac
