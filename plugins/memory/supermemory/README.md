...
|-----|---------|-------------|
| `container_tag` | `linket` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `linket-{identity}` → `linket-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Linket profile:

```json
{
  "container_tag": "linket-{identity}"
}
```

For a profile named `coder`, this resolves to `linket-coder`. The default profile resolves to `linket-default`. Without `{identity}`, all profiles share the same container.
