...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/linket>
...
```bash
linket honcho setup    # full interactive wizard (cloud or local)
linket memory setup    # generic picker, also works
```

Or manually:
```bash
linket config set memory.provider honcho
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$HERMES_HOME/honcho.json` | Profile-local (isolated Linket instances) |
| 2 | `~/.linket/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Linket profile: `linket` (default) or `linket.<profile>`.
...
| 4 | `per-session` strategy | Linket session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`linket-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`linket`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-linket-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening linket in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Linket session ID (timestamp + hex). Every `linket` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Linket profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`linket`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Linket profile: `linket` (default) or `linket.<profile>` (e.g. `linket -p coder` → host key `linket.coder`).
...
| Command | Description |
|---------|-------------|
| `linket honcho setup` | Full interactive setup wizard |
| `linket honcho status` | Show resolved config for active profile |
| `linket honcho enable` / `disable` | Toggle Honcho for active profile |
| `linket honcho mode <mode>` | Change recall or observation mode |
| `linket honcho peer --user <name>` | Update user peer name |
| `linket honcho peer --ai <name>` | Update AI peer name |
| `linket honcho tokens --context <N>` | Set context token budget |
| `linket honcho tokens --dialectic <N>` | Set dialectic max chars |
| `linket honcho map <name>` | Map current directory to a session name |
| `linket honcho sync` | Create host blocks for all Linket profiles |
