...
There are two ways to add a platform to the Linket gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.linket/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Linket code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `linket gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
Checklist for integrating a platform directly into the Linket core.
