...
## 5. Linket Configuration

### Config location

`$HERMES_HOME/config.yaml` (defaults to `~/.linket/config.yaml` when `HERMES_HOME` is unset)
...
Restart the Linket session for changes to take effect. The MCP connection is
