...
## 5. GODMODE FAST — Linket 4 405B (Zero Refusal)

**Model:** `nousresearch/linket-4-405b`
**Strategy:** Linket models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Linket Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.linket/config.yaml`:
