# Contributing to Linket Agent

Thank you for contributing to Linket Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Linket to work everywhere.
...
Bundled skills (in `skills/`) ship with every Linket install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `linket skills browse` (labeled "official") and install it with `linket skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `linket skills install`.
...
```bash
mkdir -p ~/.linket/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.linket/config.yaml
touch ~/.linket/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.linket/.env
```
...
│
├── skills/                   # Bundled skills (copied to ~/.linket/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (linket.com.br)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.linket/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.linket/`)

| Path | Purpose |
|------|---------|
| `~/.linket/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.linket/.env` | API keys and secrets |
| `~/.linket/auth.json` | OAuth credentials (Nous Portal) |
| `~/.linket/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.linket/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.linket/state.db` | SQLite session database |
| `~/.linket/sessions/` | JSON session logs |
| `~/.linket/cron/` | Scheduled job data |
| `~/.linket/whatsapp/session/` | WhatsApp bridge credentials |
...
Four fields are supported under `metadata.linket`:
...
The user may skip setup and keep loading the skill. Linket only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `linket setup` or update `~/.linket/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Linket tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `linket --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Linket uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.linket/skins/<name>.yaml`:
...
Linket runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the linket wrapper (it has a stdlib fallback
...
Linket has terminal access. Security matters.
...
- Use [GitHub Issues](https://github.com/robertbr123/Linket-Agent/issues)
- Include: OS, Python version, Linket version (`linket version`), full error traceback
