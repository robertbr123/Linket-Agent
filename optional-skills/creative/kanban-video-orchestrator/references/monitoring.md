...
linket kanban watch --tenant <project-slug>

# Snapshot of the board
linket kanban list --tenant <project-slug>
linket kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
linket kanban stats --tenant <project-slug>

# Visual dashboard (browser)
linket dashboard

# Inspect a specific task (includes comments + events)
linket kanban show <task-id>

# Follow a single task's event stream
linket kanban tail <task-id>
```

Verify available subcommands with `linket kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `linket kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `linket kanban block <id>` then `linket kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `linket kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `linket kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `linket profile list` (profile exists?), `linket status` (gateway/dispatcher up?), and verify tenant. |
...
linket kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
linket kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(linket kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
linket kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
linket kanban block <task-id>
linket kanban archive <task-id>

# Diagnose what happened
linket kanban show <task-id>      # task body, comments, recent events
linket kanban tail <task-id>      # follow the live event stream
linket kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `linket kanban create` for the director
...
while true; do
    clear
    linket kanban list --tenant <slug>
    echo "---"
    linket kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `linket kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `linket profile list` and start
