...
Maps each role archetype to the Linket skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public linket-agent
...
## Linket skills relevant to video production

### Visual / rendering skills (`linket-agent/skills/creative/`)
...
### Design / pre-production skills (`linket-agent/skills/creative/`)
...
### Audio / media skills (`linket-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`linket-agent/skills/devops/`)
...
These are **not** Linket skills but external CLIs / APIs that profiles invoke.
...
## Built-in Linket tools for media review

These are native Linket tools — not invoked via terminal but through their own
...
key is present in `~/.linket/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Linket profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.linket/.env` → environment variable.
...
When a new Linket-public video skill ships:
