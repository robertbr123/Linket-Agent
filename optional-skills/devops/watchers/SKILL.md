...
Watch a GitHub repo (set `GITHUB_TOKEN` in `~/.linket/.env` to avoid the 60 req/hr anonymous rate limit):

```bash
python $HERMES_HOME/skills/devops/watchers/scripts/watch_github.py \
  --name linket-issues --repo NousResearch/linket-agent --scope issues
```
